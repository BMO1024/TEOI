from test_functions.test_Trigonometric_Integrals import test_coshint, test_ei, test_expint, test_sinhint, test_eulergamma, test_cosint, test_sinint, test_logint, test_ssi
from test_functions.test_Elliptic_Integrals import test_ellipke, test_ellipticCE, test_ellipticCK, test_ellipticCPi, test_ellipticK, test_ellipticE, test_ellipticF
import numpy as np
import scipy
    
if __name__ == '__main__':

    # testing Trigonometric_Integrals
    # test_coshint()
    # test_ei()
    # test_expint()
    # test_sinhint()
    # test_eulergamma()
    # test_cosint()
    # test_sinint()
    # test_logint()
    # test_ssi()

    # testing Elliptic_Integrals
    # test_ellipke()
    # test_ellipticCE()
    # test_ellipticCK()
    # test_ellipticCPi()
    test_ellipticK()
    # test_ellipticE()
    test_ellipticF()
